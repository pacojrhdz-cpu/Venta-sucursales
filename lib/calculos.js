...
// Semanas naturales (lun-dom) cuyo DOMINGO cae en el mes dado.
//  -> [{ inicioISO, finISO }]  (7 dias cada una)
export function semanasNaturales(anio, mes) {
  ...
  for (let d = 1; d <= total; d++) {
    ...
    if (dow === 6) {
      const finISO = _fmt(fecha);
      out.push({ inicioISO: sumarDias(finISO, -6), finISO });
    }
  }
  return out;
}
