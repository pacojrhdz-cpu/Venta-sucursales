...
// ---- ¿El colaborador cuenta en esta semana? ----
// Sí si está activo, o si (aunque esté inactivo) tiene registro de nómina o de
// días trabajados en esa semana (o sea, esa semana sí se le pagó). Así los
// inactivos siguen contando su historial pero no se auto-suman en semanas nuevas.
export function cuentaEnSemana(cid, weekStart, activo, regMes = {}, registros = {}) {
  if (activo) return true;
  return !!(regMes[`${cid}|${weekStart}`] || registros[`${cid}|${weekStart}`]);
}
...
export function nominaMensual({ anio, mes, metaMensual, ventas, colaboradores, registros = {}, regMes = {}, deducciones = {}, cfg }) {
  ...
  tramos.forEach((t, idx) => {
    ...
    // Solo colaboradores que cuentan en esa semana (activos o con registro)
    const colsSem = (colaboradores || []).filter(c => cuentaEnSemana(c.id, t.start, c.activo, regMes, registros));
    const cols = colsSem.map(c => ({ id: c.id, retardos: retDe(c.id, t.start), faltas: 0,
...
// ---- Sueldo efectivo de una semana (INDEPENDIENTE) ----
// Usa el sueldo capturado para ESA semana; si no tiene, el sueldo base del
// colaborador. No toma valores de otras semanas (editar una no afecta las demás).
// regMes: { 'cid|start': {sueldo, metodo} }.
export function sueldoEfectivoSemana(cid, idx, tramos, regMes, base) {
  const r = regMes[`${cid}|${tramos[idx]?.start}`];
